...
const T = {
  en: {
    ...
    // Login Modal
    loginTitle: 'Login', loginSub: 'Track your pet\'s health',
    googleLogin: 'Google', appleLogin: 'Apple',
    kakaoLogin: 'Kakao', naverLogin: 'Naver',
    orDivider: 'or', emailPlaceholder: 'Email', passwordPlaceholder: 'Password',
    namePlaceholder: 'Name',
    loginBtn: 'Sign In', signupLink: 'No account? Sign Up',
    loginEmailTab: 'Login', signupEmailTab: 'Sign Up',
    // Ad
  ...
  ko: {
    ...
    loginTitle: '로그인', loginSub: '반려동물 건강 추적',
    googleLogin: 'Google', appleLogin: 'Apple',
    kakaoLogin: '카카오', naverLogin: '네이버',
  ...
  ja: {
    ...
    loginTitle: 'ログイン', loginSub: 'ペットの健康管理',
    googleLogin: 'Google', appleLogin: 'Apple',
    kakaoLogin: 'Kakao', naverLogin: 'Naver',
...
function shareAnalysis() {
  ...
  const avgScore = recent.length > 0 ? Math.round(recent.reduce((s, h) => s + h.score, 0) / recent.length) : 0;
  const pet = DB.pets.find(p => p.id === DB.activePet);
  const petName = pet ? pet.name : (isKo ? '반려동물' : 'Pet');
  const report = generateHealthReport(state.reportPeriod || 'weekly');

  const shareText = `🐾 PoopBuddy ${isKo ? '건강 리포트' : 'Health Report'}
━━━━━━━━━━━━━━━━
${isKo ? '반려동물' : 'Pet'}: ${petName}
${isKo ? '평균 건강 점수' : 'Avg Score'}: ${avgScore}/100
${isKo ? '기간' : 'Period'}: ${isKo ? '최근 7일' : 'Last 7 days'}
${isKo ? '총 기록' : 'Total Logs'}: ${report.totalRecords}${isKo ? '회' : ''}
${isKo ? 'AI 추천' : 'Recommendation'}: ${report.recommendation}
━━━━━━━━━━━━━━━━
${recent.slice(-5).map(h => {
    const d = new Date(h.date);
    return `${d.getMonth() + 1}/${d.getDate()} — ${isKo ? '점수' : 'Score'}: ${h.score}, Bristol: Type ${h.bristol}`;
  }).join('\n')}
━━━━━━━━━━━━━━━━
${isKo ? '※ 이 리포트는 PoopBuddy 앱에서 자동 생성되었습니다.' : '※ Generated by PoopBuddy App'}`;

  if (navigator.share) {
    navigator.share({
      title: `PoopBuddy ${isKo ? '건강 리포트' : 'Health Report'} — ${petName}`,
      text: shareText,
    }).then(() => {
      showToast('✅ ' + (isKo ? '공유 완료!' : isJa ? '共有完了！' : 'Shared!'));
    }).catch(e => {
      if (e.name !== 'AbortError') showToast('❌ ' + (isKo ? '공유 실패' : 'Share failed'));
    });
  } else {
    navigator.clipboard.writeText(shareText).then(() => {
      showToast('📋 ' + (isKo ? '리포트가 클립보드에 복사됨!' : 'Report copied to clipboard!'));
    }).catch(() => {
      showToast('❌ ' + (isKo ? '복사 실패' : 'Copy failed'));
    });
  }
}

// ── 분석 피드백 루프 (사용자 교정 → 로컬 학습) ──
function submitAnalysisFeedback(analysisId, originalScore, originalBristol) {
  const isKo = state.lang === 'ko';
  const isJa = state.lang === 'ja';
  ...
  modal.className = 'modal-overlay';
  modal.innerHTML = `
    <div class="daily-log-modal" style="max-width:360px">
      <button onclick="document.getElementById('feedbackModal').remove()" style="position:absolute;top:12px;right:16px;background:none;font-size:1.2rem;color:var(--text-muted);border:none;cursor:pointer">✕</button>
      <h3 style="margin-bottom:16px;font-size:1.1rem">🔄 ${isKo ? '분석 결과 교정' : isJa ? '分析結果の修正' : 'Correct Analysis'}</h3>
      <p style="font-size:0.85rem;color:var(--text-secondary);margin-bottom:16px">${isKo ? '틀린 부분을 알려주세요. 다음 분석에 반영됩니다!' : isJa ? '間違いを教えてください。次の分析に反映されます！' : 'Tell us what was wrong. It improves future analysis!'}</p>
      <div class="log-field" style="margin-bottom:12px">
        <label style="font-weight:600;margin-bottom:6px;display:block">📊 ${isKo ? '실제 건강 상태' : 'Actual Health'}</label>
        <div class="log-radio-group">
          <button class="log-radio active" data-field="fbSeverity" data-value="good" onclick="document.querySelectorAll('[data-field=fbSeverity]').forEach(b=>b.classList.remove('active'));this.classList.add('active')">😊 ${isKo ? '좋음' : 'Good'}</button>
          <button class="log-radio" data-field="fbSeverity" data-value="caution" onclick="document.querySelectorAll('[data-field=fbSeverity]').forEach(b=>b.classList.remove('active'));this.classList.add('active')">😐 ${isKo ? '주의' : 'Caution'}</button>
          <button class="log-radio" data-field="fbSeverity" data-value="warning" onclick="document.querySelectorAll('[data-field=fbSeverity]').forEach(b=>b.classList.remove('active'));this.classList.add('active')">😟 ${isKo ? '경고' : 'Warning'}</button>
        </div>
      </div>
      <div class="log-field" style="margin-bottom:12px">
        <label style="font-weight:600;margin-bottom:6px;display:block">💩 ${isKo ? '실제 Bristol 타입' : 'Actual Bristol Type'}</label>
        <div class="log-radio-group" style="flex-wrap:wrap">
          ${[1, 2, 3, 4, 5, 6, 7].map(b => `<button class="log-radio ${b === originalBristol ? 'active' : ''}" data-field="fbBristol" data-value="${b}" onclick="document.querySelectorAll('[data-field=fbBristol]').forEach(b=>b.classList.remove('active'));this.classList.add('active')" style="min-width:40px">T${b}</button>`).join('')}
        </div>
      </div>
      <div class="log-field" style="margin-bottom:16px">
        <label style="font-weight:600;margin-bottom:6px;display:block">📝 ${isKo ? '추가 의견 (선택)' : 'Additional notes (optional)'}</label>
        <input id="fbComment" type="text" class="log-input" placeholder="${isKo ? '어떤 점이 틀렸나요?' : 'What was incorrect?'}">
      </div>
      <button class="btn-primary" style="width:100%;justify-content:center" onclick="saveFeedback('${analysisId}', ${originalScore})">
        ${isKo ? '✅ 교정 제출' : isJa ? '✅ 修正を送信' : '✅ Submit Correction'}
      </button>
    </div>`;
  modal.addEventListener('click', e => { if (e.target === modal) modal.remove(); });
  document.body.appendChild(modal);
...
function saveFeedback(analysisId, originalScore) {
  const isKo = state.lang === 'ko';
  const correctedSeverity = document.querySelector('[data-field="fbSeverity"].active')?.dataset?.value || 'good';
  const correctedBristol = parseInt(document.querySelector('[data-field="fbBristol"].active')?.dataset?.value || '4');
  const comment = document.getElementById('fbComment')?.value || '';

  // 피드백을 localStorage에 저장
  const feedbackHistory = JSON.parse(localStorage.getItem('pb-feedback-history') || '[]');
  ...
  localStorage.setItem('pb-feedback-history', JSON.stringify(feedbackHistory));

  document.getElementById('feedbackModal')?.remove();
  haptic('success');
  showToast(isKo ? '✅ 교정 완료! 다음 분석에 반영됩니다' : '✅ Feedback saved! Will improve next analysis');
  addXP(5, isKo ? '피드백 제출' : 'Feedback');
}

// 분석 시 피드백 히스토리 가져오기 (프롬프트에 전달용)
function getFeedbackHistory() {
  return JSON.parse(localStorage.getItem('pb-feedback-history') || '[]');
}

// ── 가까운 동물병원 찾기 (Google Maps 검색 링크 — 무료) ──
function openNearbyVetMap() {
  const isKo = state.lang === 'ko';
  const isJa = state.lang === 'ja';
  const searchQuery = isKo ? '동물병원' : isJa ? '動物病院' : 'veterinary clinic';

  if (navigator.geolocation) {
    showToast(isKo ? '📍 위치 확인 중...' : '📍 Getting location...');
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        const { latitude, longitude } = pos.coords;
        // Google Maps 검색 URL (무료 링크)
        const mapUrl = `https://www.google.com/maps/search/${encodeURIComponent(searchQuery)}/@${latitude},${longitude},14z`;
        window.open(mapUrl, '_blank');
      },
      (err) => {
        // GPS 실패 시 검색어만으로 맵 열기
        console.warn('[VetMap] 위치 가져오기 실패:', err.message);
        const mapUrl = `https://www.google.com/maps/search/${encodeURIComponent(searchQuery)}`;
        window.open(mapUrl, '_blank');
        showToast(isKo ? '📍 위치 없이 검색합니다' : '📍 Searching without location');
      },
...
function showLoginModal() {
  ...
  if (isLoggedIn) {
    ...
    const btnStyle = 'width:100%;padding:11px 14px;border:none;border-radius:10px;font-size:0.9rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:8px;justify-content:center;margin-bottom:6px;transition:transform 0.1s,box-shadow 0.2s;';
    modal.innerHTML = `
      <div style="background:var(--bg-card);border-radius:var(--radius-xl);padding:24px;max-width:360px;width:88%;position:relative;max-height:90vh;overflow-y:auto">
        <button onclick="document.getElementById('loginModal').remove()" style="position:absolute;top:8px;right:12px;background:none;font-size:1.2rem;color:var(--text-muted);cursor:pointer;z-index:1">✕</button>
        <div style="text-align:center;margin-bottom:16px">
          <div style="font-size:2rem;margin-bottom:4px">🐾</div>
          <h2 style="margin-bottom:2px;font-size:1.15rem">${t('loginTitle')}</h2>
          <p style="color:var(--text-secondary);font-size:0.8rem">${t('loginSub')}</p>
        </div>
        ...
        <button onclick="socialLogin('kakao')" style="${btnStyle}background:#FEE500;color:#191919;">
          <svg width="16" height="16" viewBox="0 0 24 24"><path d="M12 3C6.5 3 2 6.58 2 11c0 2.84 1.87 5.33 4.67 6.74l-1.2 4.46c-.1.36.32.65.63.44l5.23-3.46c.22.02.44.03.67.03 5.5 0 10-3.58 10-8s-4.5-8-10-8z" fill="#191919"/></svg>
          ${t('kakaoLogin')}
        </button>

        <button onclick="socialLogin('naver')" style="${btnStyle}background:#03C75A;color:#fff;">
          <svg width="16" height="16" viewBox="0 0 24 24"><path d="M16.27 3H7.73v7.31L4 3H0v18h7.73v-7.31L11.46 21H16.27V3z" fill="#fff" transform="translate(4,3) scale(0.75)"/></svg>
          ${t('naverLogin')}
        </button>
        ...
        <div style="display:flex;align-items:center;margin:14px 0;gap:10px">
          ...
          <span style="color:var(--text-muted);font-size:0.75rem">${t('orDivider')}</span>
          <div style="flex:1;height:1px;background:var(--border)"></div>
        </div>

        <!-- Email/Password Form -->
        <div id="authModeToggle" style="display:flex;gap:6px;margin-bottom:10px">
          <button id="authLoginTab" onclick="switchAuthMode('login')" style="flex:1;padding:7px;border:none;border-radius:8px;background:var(--accent);color:#fff;font-weight:600;cursor:pointer;font-size:0.8rem">${t('loginEmailTab')}</button>
          <button id="authSignupTab" onclick="switchAuthMode('signup')" style="flex:1;padding:7px;border:none;border-radius:8px;background:var(--bg-secondary);color:var(--text-secondary);font-weight:600;cursor:pointer;font-size:0.8rem">${t('signupEmailTab')}</button>
        </div>
        <div id="nameFieldWrap" style="display:none">
          <input type="text" id="loginName" placeholder="${t('namePlaceholder')}" style="width:100%;padding:10px 14px;border:1px solid var(--border);border-radius:10px;font-family:var(--font);margin-bottom:6px;background:var(--bg-secondary);color:var(--text-primary);box-sizing:border-box;font-size:0.9rem">
        </div>
        <input type="email" id="loginEmail" placeholder="${t('emailPlaceholder')}" style="width:100%;padding:10px 14px;border:1px solid var(--border);border-radius:10px;font-family:var(--font);margin-bottom:6px;background:var(--bg-secondary);color:var(--text-primary);box-sizing:border-box;font-size:0.9rem">
        <input type="password" id="loginPassword" placeholder="${t('passwordPlaceholder')}" style="width:100%;padding:10px 14px;border:1px solid var(--border);border-radius:10px;font-family:var(--font);margin-bottom:12px;background:var(--bg-secondary);color:var(--text-primary);box-sizing:border-box;font-size:0.9rem">
        <div id="authError" style="display:none;color:#ff4444;font-size:0.8rem;margin-bottom:8px;text-align:center"></div>
        <button class="btn-primary" style="width:100%;justify-content:center;font-size:0.95rem;padding:11px" id="loginSubmitBtn">${t('loginBtn')}</button>
      </div>`;
    setTimeout(() => {
      // Auth mode state
...
async function runAnalysis() {
  ...
  document.getElementById('analysisResult').innerHTML = `
    <div class="card">
      ...
      ${(() => {
      // 기생충 의심 경고 카드
      const parasiteRisk = aiResult ? (aiResult.parasiteRisk || 'none') : 'none';
      const parasiteDetails = aiResult ? (aiResult.parasiteDetails || '') : '';
      const isKoP = state.lang === 'ko';
      const isJaP = state.lang === 'ja';
      if (parasiteRisk !== 'none') {
        const riskLabelMap = { low: isKoP ? '낮음' : 'Low', moderate: isKoP ? '중간' : 'Moderate', high: isKoP ? '높음' : 'High' };
        const riskColorMap = { low: '#FF9800', moderate: '#F44336', high: '#D32F2F' };
        return `
          <div style="background:rgba(244,67,54,0.06);border:2px solid ${riskColorMap[parasiteRisk]};border-radius:14px;padding:18px;margin-top:14px">
            <h4 style="margin:0 0 10px 0;font-size:1.05rem;color:${riskColorMap[parasiteRisk]}">🪱 ${isKoP ? '기생충 의심 경고' : isJaP ? '寄生虫の疑い警告' : 'Parasite Suspicion Alert'}</h4>
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
              <span style="display:inline-block;padding:3px 10px;border-radius:8px;font-size:0.8rem;font-weight:700;background:${riskColorMap[parasiteRisk]}20;color:${riskColorMap[parasiteRisk]}">${isKoP ? '위험도' : 'Risk'}: ${riskLabelMap[parasiteRisk]}</span>
            </div>
            ${parasiteDetails ? `<p style="font-size:0.85rem;color:var(--text-primary);line-height:1.6;margin:0 0 12px 0">${parasiteDetails}</p>` : ''}
            <div style="background:rgba(255,152,0,0.08);border-radius:10px;padding:12px;margin-bottom:12px">
              <p style="font-size:0.85rem;color:var(--text-primary);margin:0;line-height:1.6">
                ${isKoP ? '⚠️ <strong>이것은 의료 진단이 아닙니다.</strong> AI가 시각적 단서로 감지한 의심 소견입니다. 정확한 진단을 위해 반드시 수의사의 <strong>분변검사(현미경 검사)</strong>를 받으세요.' : '⚠️ <strong>This is NOT a medical diagnosis.</strong> These are visual cues detected by AI. Please get a proper <strong>fecal examination</strong> from your veterinarian.'}
              </p>
            </div>
            <button onclick="openNearbyVetMap()" class="btn-primary" style="width:100%;justify-content:center;background:linear-gradient(135deg,#F44336,#E91E63)">
              🏥 ${isKoP ? '가까운 동물병원 찾기' : isJaP ? '近くの動物病院を探す' : 'Find Nearby Vet'}
            </button>
          </div>`;
      }
      return '';
      ...
      <div class="result-actions" style="margin-top:20px;display:flex;flex-direction:column;gap:10px">
        ...
        <button class="btn-secondary" id="feedbackBtn" style="width:100%;justify-content:center;border-color:var(--accent-peach,#FF9800);color:var(--accent-peach,#FF9800)">
          🔄 ${state.lang === 'ko' ? '분석 틀렸어요 (교정하기)' : state.lang === 'ja' ? '分析が間違っています（修正）' : 'Wrong analysis? (Correct it)'}
        </button>
  ...
  // Wire up feedback button (분석 교정)
  const feedbackBtn = document.getElementById('feedbackBtn');
  if (feedbackBtn) {
    feedbackBtn.onclick = () => {
      submitAnalysisFeedback(analysisRecord.date, r.score, bristol);
    };
  }
}
